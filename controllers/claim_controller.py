...
def get_claims():
    ...
    if current_user_role() == "MANAGER":
        ...
        if search:
            ...
            claims = [claim for claim in claims if term in f"{claim.claim_number} {claim.description or ''}".lower()]
    elif search:
...
def create_claim():
    try:
        ...
        require_json_fields(data, ("claim_number",))
...
def update_claim_status(claim_id):
    try:
        ...
        if current_user_role() == "FINANCE" and next_status not in {"VERIFIED", "REJECTED"}:
...
def update_claim(claim_id):
    try:
        ...
        update_allowed(claim, get_payload(), {"travel_request_id", "description", "total_amount"})
